...
## Unique per-ability art (res://assets/player/ability_*.png, generated by
## tools/gen_ability_art.py) falls back to the player's own battle sprite
## if it hasn't been generated yet -- same ResourceLoader.exists() guard
## pattern as CardDatabase._load_ability_art().
func _load_ability_art(path: String, fallback: Texture2D) -> Texture2D:
...
func _build_standalone_ability_cards() -> Array[BaseCardData]:
	...
	rock_ability.art_texture = _load_ability_art("res://assets/player/ability_rock_throw.png", texture)
	var rock_card := AbilityCardData.new()
	rock_card.ability = rock_ability
	rock_card.card_name = "Rock Throw"
	rock_card.art_texture = rock_ability.art_texture
	...
	brace_ability.art_texture = _load_ability_art("res://assets/player/ability_brace.png", texture)
	var brace_card := AbilityCardData.new()
	brace_card.ability = brace_ability
	brace_card.card_name = "Brace"
	brace_card.art_texture = brace_ability.art_texture
